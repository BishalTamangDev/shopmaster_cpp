...
public:
    ...
    bool fetch(int, Product &) const;                       // fetch product
    bool update(int, Product &);                            // update product details
    bool remove(int);                                       // remove product
    bool restock(int, int);                                 // restock
    bool searchById(int, Product &) const;                  // search by id
    std::vector<Product> searchByName(std::string &) const; // search by name
    bool isValidProductName(std::string &) const;           // check if the product name is already taken
    int getValidId() const;                                 // get valid id
